    ...
    document.addEventListener('DOMContentLoaded', initMap);
        ...
        function initCharts() {
            ...
            if (pyramidCanvas) {
                ...
                const malePopulation = [-37, -62, -77, -62, -70, -67, -47, -61, -63, -58, -56, -61, -58, -39, -26, -19, -11, -19];
                const femalePopulation = [39, 48, 63, 66, 93, 76, 50, 52, 65, 72, 47, 68, 39, 44, 26, 17, 9, 20];
...
// Tingkat Pendidikan
        function initEducationChart() {
            ...
            if (educationCanvas) {
                ...
                // Data baru berdasarkan input Anda
                    const educationLevels = ['Tidak Sekolah', 'TK', 'SD', 'SMP', 'SMA', 'D3', 'D4', 'S1', 'S2'];
                    const educationData = [602, 155, 451, 214, 276, 5, 4, 80, 1];
    
                // Warna yang konsisten dengan tema
                const backgroundColors = [
                    'rgba(78, 115, 223, 0.7)',    // Tidak Sekolah - Biru
                    'rgba(28, 200, 138, 0.7)',    // SD - Hijau
                    'rgba(54, 185, 204, 0.7)',    // SMP - Cyan
                    'rgba(246, 194, 62, 0.7)',    // SMA - Kuning
                    'rgba(231, 74, 59, 0.7)',     // S1 - Merah
                    'rgba(111, 66, 193, 0.7)',    // TK - Ungu
                    'rgba(133, 135, 150, 0.7)',   // D3 - Abu-abu
                    'rgba(27, 156, 228, 0.7)',    // D4 - Biru Muda
                    'rgba(255, 159, 64, 0.7)'    // S2 - Oranye
                ];
                    ...
                    options: {
                        ...
                        plugins: {
                            tooltip: {
                                callbacks: {
                                    label: function(context) {
                                            const total = educationData.reduce((a, b) => a + b, 0);
                                            const percentage = ((context.raw / total) * 100).toFixed(1);
                                        
                                        return `Jumlah: ${context.raw} orang (${percentage}%)`;
                                    }
                                }
                            },
                            legend: {
                                display: false // Sembunyikan legend karena sudah ada label di sumbu X
                            }
                        },
                        animation: {
                            duration: 1000,
        ...
        // Initialize Jenis Pekerjaan Chart
function initJenisPekerjaanChart() {
    const ctx = document.getElementById('jenisPekerjaanChart');
    if (!ctx) return;
    
    // Data dikelompokkan untuk visualisasi yang lebih jelas
    const labels = [
        'Tidak Bekerja', 
        'Petani', 
        'Pelajar/Mahasiswa', 
    ...
    const data = [534, 409, 322, 261, 153, 108]; // 153 = Wiraswasta(53+27) + Tukang(69+4), 108 = lainnya
    const total = data.reduce((a, b) => a + b, 0);
    
    const backgroundColors = [
        'rgba(133, 135, 150, 0.8)',  // Tidak Bekerja - Abu
        'rgba(28, 200, 138, 0.8)',   // Petani - Hijau
        'rgba(78, 115, 223, 0.8)',   // Pelajar - Biru
        'rgba(246, 194, 62, 0.8)',   // Rumah Tangga - Kuning
        'rgba(231, 74, 59, 0.8)',    // Wirausaha - Merah
        'rgba(111, 66, 193, 0.8)'    // Lainnya - Ungu
    ];
    ...
    new Chart(ctx, {
        ...
        // Panggil inisialisasi chart pendidikan dari dalam initCharts
        initEducationChart();

        // Panggil inisialisasi chart jenis pekerjaan
        initJenisPekerjaanChart();


    } // Tutup function initCharts

// Jalankan fungsi inisialisasi ketika halaman selesai dimuat
document.addEventListener('DOMContentLoaded', function() {
    // Pastikan DOM siap lalu inisialisasi chart
    initCharts();
});
